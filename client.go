...
func dialTimeout(f newClientFunc, network, addr string, opts ...*Option) (client *Client, err error) {
	...
	case <-time.After(opt.ConnectTimeout):
		return nil, fmt.Errorf("rpc client: connect timeout: expeceted within %s", opt.ConnectTimeout)
	case result := <-ch:
		return result.client, result.err
	}
}
...
func XDial(addr string, opts ...*Option) (*Client, error) {
	...
	default:
		return Dial(protocol, address, opts...)
	}
}
